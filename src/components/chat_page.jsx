...
const mapDirectConversation = (item) => ({
    id: `direct-${item.userId}`,
    type: "direct",
    targetId: item.userId,
    name: item.name || item.userid || item.email || "사용자",
    charge: item.job || item.email || "친구",
    lastMsg: item.lastMessage || "대화를 시작해 보세요.",
    time: formatTime(item.lastTimestamp),
    state: item.presenceStatus || "OFFLINE",
    raw: item,
});

const mapGroupConversation = (item) => ({
