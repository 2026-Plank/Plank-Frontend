...
const TodoPanel = styled.section`
  display: flex;
  flex-direction: column;
  gap: 14px;
  width: 100%;
...
const TodoProgressRow = styled.div`
  display: flex;
  align-items: center;
...
const TodoProgressText = styled.span`
  color: #70716f;
  font-size: 14px;
  font-weight: 600;
`;
...
const TodoProgressFill = styled.div`
...
const TodoCheckBtn = styled.button`
  ...
  flex-shrink: 0;
  cursor: pointer;
  background: ${({ $done }) => ($done ? '#c0da58' : '#e2e2e2')};
...
const TodoAddBtn = styled.button`
  ...
  &:disabled { background: #c9c9c8; cursor: not-allowed; }
`;
...
const FeedbackTargetSelect = styled.select`
  ...
  background-image: url("data:image/svg+xml,%3Csvg width='16' height='10' viewBox='0 0 16 10' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M3 3L8 8L13 3' stroke='%23222' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");
...
export default function TeamDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadTodos = async () => {
      try {
        const data = await apiRequest(`/api/teams/${teamId}/todos`);
        setProjectTodos(Array.isArray(data?.todos) ? data.todos : []);
      } catch (error) {
        console.error("투두를 불러오지 못했습니다.", error);
      }
    };
  ...
  const todoTotal = projectTodos.length;
  const todoDone = projectTodos.filter((t) => t.status === "Done").length;
  const todoProgress = todoTotal > 0 ? Math.round((todoDone / todoTotal) * 100) : 0;

  const handleAddTodo = async () => {
    const title = newTodoText.trim();
    if (!title || !normalizedTeam.id) return;
    setTodoLoading(true);
    try {
      const data = await apiRequest(`/api/teams/${normalizedTeam.id}/todos`, {
        method: "POST",
        body: JSON.stringify({ title }),
      });
      if (data?.todo) setProjectTodos((prev) => [...prev, data.todo]);
      setNewTodoText("");
    } catch (error) {
  ...
  const handleToggleTodo = async (todo) => {
    const nextStatus = todo.status === "Done" ? "Wait" : "Done";
    setProjectTodos((prev) => prev.map((t) => (t.id === todo.id ? { ...t, status: nextStatus } : t)));
    try {
      await apiRequest(`/api/teams/${normalizedTeam.id}/todos/${todo.id}`, {
        method: "PATCH",
        body: JSON.stringify({ status: nextStatus }),
      });
    } catch (error) {
      setProjectTodos((prev) => prev.map((t) => (t.id === todo.id ? { ...t, status: todo.status } : t)));
    }
  };
  ...
  const handleDeleteTodo = async (todoId) => {
    ...
    try {
      await apiRequest(`/api/teams/${normalizedTeam.id}/todos/${todoId}`, { method: "DELETE" });
    } catch (error) {
      const data = await apiRequest(`/api/teams/${normalizedTeam.id}/todos`);
      setProjectTodos(Array.isArray(data?.todos) ? data.todos : []);
    }
        ...
        <ContentBox>
          ...
          <Wapper>
            ...
            <BottomWapper>
              <TextWapper>
                <VerticalLine />
                <DescriptionText>할 일 목록</DescriptionText>
              </TextWapper>

              <TodoPanel>
                <TodoProgressRow>
                  <TodoProgressText>완성도</TodoProgressText>
                  <TodoProgressText>{todoProgress}% ({todoDone}/{todoTotal})</TodoProgressText>
                </TodoProgressRow>
                <TodoProgressBar>
                  <TodoProgressFill $progress={todoProgress} />
                </TodoProgressBar>
                ...
                <TodoList>
                  ...
                  {projectTodos.map((todo) => (
                    <TodoRow key={todo.id}>
                      ...
                      />
                      <TodoItemText $done={todo.status === "Done"}>{todo.title}</TodoItemText>
                      <TodoDeleteBtn type="button" onClick={() => handleDeleteTodo(todo.id)}>✕</TodoDeleteBtn>
                    </TodoRow>
                  ))}
                </TodoList>
                ...
                <TodoAddRow>
                  <TodoInput
                    ...
                    disabled={!normalizedTeam.id}
                  />
                  <TodoAddBtn
                    type="button"
                    onClick={handleAddTodo}
                    disabled={todoLoading || !newTodoText.trim() || !normalizedTeam.id}
                  >
                    추가
                  </TodoAddBtn>
                </TodoAddRow>
