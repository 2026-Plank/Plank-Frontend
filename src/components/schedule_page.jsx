...
export default function SchedulePage() {
    ...
    const loadSchedules = async () => {
        ...
        try {
            ...
            setError(loadError.data?.error || loadError.message || "일정을 불러오지 못했습니다.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const visibleSchedules = useMemo(() => schedules.filter((item) => item.type !== "ProjectTodo"), [schedules]);

    const scheduleCounts = useMemo(() => {
        const counts = { "모든 일정": visibleSchedules.length };
    ...
    const resetForm = (nextType = initialForm.type, nextDate = selectedDate || toDateKey(today)) => {
        setEditingId(null);
        setForm({
            ...initialForm,
            type: nextType,
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editingId) {
                ...
                setMessage("일정이 수정되었어요.");
            } else {
                await apiRequest("/api/schedules", {
                    method: "POST",
                    body: JSON.stringify(payload),
                });
                setMessage("일정이 추가되었어요.");
            }
            await loadSchedules();
            setSelectedDate(form.targetDate);
            resetForm(form.type, form.targetDate);
        } catch (submitError) {
            setError(submitError.data?.error || submitError.message || "\uC77C\uC815 \uC800\uC7A5\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.");
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            setError(deleteError.data?.error || deleteError.message || "\uC77C\uC815 \uC0AD\uC81C\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.");
        }
    };
